...
\item Construct the following TM, $H$:
  ...
  \item $H$ decides the language $Halt$.
\item But that's impossible!
\end{itemize}
...
\item  $\{\langle M\rangle\mid \mbox{$M$ accepts input of length $n^2$}\}$
\item  $\{\langle M\rangle\mid \mbox{$M$ accepts input of length $k$}\}$
\item $\{\langle M\rangle\mid \mbox{$M$ accepts $\epsilon$}\}$
...
None of these is decideable, or even enumerable.
\end{frame}
