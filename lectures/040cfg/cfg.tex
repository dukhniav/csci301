...
s = abcde
\]
with substrings $a$, $b$, $c$, $d$, $e$,  such that
\begin{enumerate}
\item $|bcd| \leq p$
\item $|bd| \geq 1$
...
\sect{Proof that $L = 0^n1^n0^n$ is not CF}
\bi
\li Suppose $L$ is CF, then $p$ exists as in the theorem.
\li $0^p1^p0^p \in L$ by definition.
\li From theorem, $0^p1^p0^p = abcde$ and $ab^ncd^ne \in L$ for all $n$, 
but we don't know which parts are where.
\li Case 1: $b$ or $d$ contains two different letters.
\bi
\li Then $ab^2cd^2e$ must have letters out of alphabetical order.
\li Then $ab^2cd^2e \not \in L$, contradiction.
\ei
\li Case 2: $b$ and $d$ each contain only one kind of letter. 
\bi 
...
\li $1^m$ where $m=n^2$
\li $1^\ell 0^m1^n$ where $\ell < m < n$
\li $1^n0^n1^i$ where $i \leq n$
